...
namespace Renoir {
	...
	public static class Util {
		...
		/// <summary>
		/// returns some basic info about a kinematic collision.
		/// </summary>
		/// <param name="collision2D"></param>
		/// <returns></returns>
		public static string DebugCollision(KinematicCollision2D collision2D) =>
			$"position={collision2D.Position} " +
			$"velocity={collision2D.ColliderVelocity} " +
			$"collider={collision2D.Collider} " +
			$"vector={collision2D.Normal} " +
			$"{collision2D.Normal.ToDirectionArrow()}";
	}
